...
ATOM MyRegisterClass(HINSTANCE hInstance, WndPara* lpsWndPara)
{
	...
	wcex.hbrBackground = (HBRUSH)(COLOR_BTNFACE + 1);//背景画刷颜色(自动)
...
BOOL InitWndInstance(HINSTANCE hInstance, int nCmdShow, WndPara* lpsWndPara, LPCALLBACKINITWNDEXTRAFUNC pCallBackInitWndExtra)
{
	...
	AdjustWindowRect(&Rect, WS_POPUP, false);//窗口具有标题栏、菜单栏、最小化窗口	//WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX
	...
	hParent = FindWindowEx(GetDesktopWindow(), NULL, L"Progman", L"Program Manager");
	hParent = FindWindowEx(hParent, NULL, L"SHELLDLL_DefView", 0);
	hParent = FindWindowEx(hParent, NULL, L"SysListView32", L"FolderView");
	...
	if (nScreenWidth > nWindowWidth && nScreenHeight > nWindowHeight)
	{
		...
		SetWindowPos(hWnd, HWND_BOTTOM, nPosX, nPosY, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);//设置窗口位置(ScreenCenter)
	}
